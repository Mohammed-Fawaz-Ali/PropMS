...
const AddPropertyForm = ({ onSuccess }) => {
  ...
  const { register, handleSubmit, watch, formState: { isSubmitting } } = useForm({
      defaultValues: {
          name: 'Golden Palms Apartment',
          address: 'Block C, Sector 15',
          city: 'Bangalore',
          state: 'Karnataka',
          pincode: '560015',
          propertyType: 'apartment', // Default to apartment for multi-unit testing
          totalUnits: 10,
          coverImageUrl: 'https://images.unsplash.com/photo-1545324418-cc1a3fa10c00?auto=format&fit=crop&w=400&q=80',
          plotAreaSqft: '',
          builtUpAreaSqft: '',
          lengthFt: '',
          widthFt: '',
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            {showUnitsInput && (
                <div>
                  ...
                  <input {...register('totalUnits')} type="number" min="1" required className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-accent bg-white" />
                </div>
            )}
          </div>
          {!showUnitsInput && (
              <p className="text-xs text-amber-600 bg-amber-50 p-2 rounded border border-amber-100">
                Residential and Commercial properties generate strictly 1 unit automatically.
              </p>
          )}
      </div>
      ...
      <div className="space-y-4">
          ...
          <div>
            <label className="block text-sm font-medium text-slate-700 mb-1">Image URL (Optional placeholder)</label>
